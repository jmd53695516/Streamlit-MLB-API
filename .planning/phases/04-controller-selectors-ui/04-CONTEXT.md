...
- **D-11:** Sort + filter data source: the **team hitting-stats endpoint**. Add a new services wrapper: `get_team_hitting_stats(team_id: int, season: int) -> list[dict]` calling `GET /teams/{team_id}/stats?stats=season&group=hitting&season={season}&sportId=1`. Returns one entry per rostered hitter with season totals (`stat.homeRuns`, `player.id`, `player.fullName`, `position.type`). One HTTP call per team selection — no per-player fan-out. Respects CLAUDE.md's "aggressive caching, no hammering."
- **D-12:** Non-pitcher filter: `primaryPosition.type != "Pitcher"` (keeps DH, OF, IF, C — also keeps any "Two-Way Player" type so Ohtani stays in). Planner/researcher should confirm the exact JSON path against a live fixture; the team-stats response may nest position under `position` or `player.primaryPosition`.
